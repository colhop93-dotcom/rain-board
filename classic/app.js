...
(function () {
  ...
  var NEXRAD = { ATL: "FFC", AUS: "EWX", BOS: "BOX", CHI: "LOT", DAL: "FWS", DC: "LWX", DEN: "FTG", EWR: "OKX", HOU: "HGX",
    ...
    SEA: "ATX", SFO: "MUX", TTN: "DIX", PIT: "PBZ" };
  ...
  /* live cameras near each gauge (data/cams_rings.json, verified entries only) */
  var cams = {}, cameraCatalogTime = null, camModalCity = null;
  function loadCams() { fetchJSON(dataURL("../data/cams_rings.json"), 6000).catch(function () { return fetchJSON(ON_MIRROR ? cdnURL("../data/cams_rings.json") : "../data/cams_rings.json", 6000); }).then(function (c) { cams = (c && c.near) || {}; cameraCatalogTime = c && c.updated_utc; }).catch(function () { }); }
  ...
  var camHealth = null;
  function loadCamHealth() { fetchJSON(dataURL("../data/cam_health.json"), 6000).then(function (h) { camHealth = h; if (openCity) refreshOpenCard(); if (camModalCity && !$("cammodal").hidden) openCams(camModalCity); }).catch(function () {}); }
  ...
  function camFresh(c) {
    ...
    if (h && (h.error || h.proxy_error)) return { k: "off", label: "UNAVAILABLE" };
    ...
    if (!h.changes) return { k: "unk", label: "no frame change observed yet" };
    return { k: "live", label: "changed " + (h.stale_min <= 1 ? "just now" : h.stale_min + " m ago") };
  }
  ...
  function freshBadge(c) { var f = camFresh(c), h = camHealth && camHealth.cams && camHealth.cams[camKey(c)];
    var times = "image capture time: " + ((h && h.source_stamp) || "unknown") + "; last successful check: " + ((h && h.last_ok_utc) || c.last_ok_utc || c.checked_utc || "unknown") + "; catalog updated: " + (cameraCatalogTime || "unknown");
    return '<small>' + esc(times) + '</small>' + (f.label ? '<span class="camfresh ' + f.k + '">' + esc(f.label) + '</span>' : ""); }
  ...
  function openCams(city) {
    ...
    var html = '<div class="camhead"><b>' + (s ? s.name : city) + '</b> cameras near the gauge <button class="chipbtn" id="camclose">close</button></div>';
  ...
  /* The nearest camera lives on the card itself (Colin, 2026-09-04: "the cards should link directly
     to the cameras that are closest automatically"). cams_rings.json is kept sorted by distance, so the
  ...
  function camSig(list) { return String(cameraCatalogTime) + String(camHealth && camHealth.updated_utc) + list.map(function (c) { return c.type + ":" + (c.id || c.frame || c.url || ""); }).join("|"); }
  function camMediaHTML(c) {
    if (camFresh(c).k === "off") return '<div class="camnone">image unavailable at the latest check</div>';
    ...
    if (c.type === "proxy") { var fu = "../" + c.frame; return '<img class="camimg" referrerpolicy="no-referrer" data-src="' + esc(fu) + '" src="' + esc(fu + '?t=' + Date.now()) + '" alt="' + esc(c.name) + '">'; }
  ...
  function buildCamNode(city) {
    ...
    el.innerHTML = '<div class="camlbl"><b>CAMERA</b> nearest verified: ' + (c.dist_km ? esc(miTxt(c.dist_km)) + ' ' + esc(c.dir || '') + ' of the gauge' : '') + (list.length > 1 ? ' <button class="chipbtn camnext">next cam (' + (i + 1) + '/' + list.length + ')</button>' : '') + '<div class="camname">' + esc(c.name) + ' ' + heldBadge(c) + '</div></div>' + camMediaHTML(c);
