...
(function (root) {
  ...
  /* Two minutes absorbs ordinary skew between the writer and the browser.
     Beyond that the stamp is not believed. This policy applies only to models. */
  ...
  var DEFS = {
    v2: {
      ...
      role: "retired headline model; retained archive identity only."
    },
    v3: {
      key: "v3", label: "V3", file: "physics_v3.json", shadow: false, sourceShadow: true,
      role: "primary board forecast, building its own live record; display does not authorize trading."
    }
  };
  ...
  function identityReason(feed, row) {
    var mid = feed && feed.model_id;
    if (typeof mid !== "string" || !mid.trim()) return "v3 feed refused: missing top-level model_id";
    if (arguments.length > 1) {
      var rid = row && row.model_id;
      if (typeof rid !== "string" || !rid.trim()) return "v3 row refused: missing model_id";
      if (rid !== mid) return "v3 row refused: model_id " + rid + " does not match feed model_id " + mid;
    }
    return null;
  }
  ...
  function ageMinOf(iso, nowMs) {
    ...
    // Check the raw age so one millisecond beyond the allowance is still refused.
    if (age < 0 && age >= -FUTURE_TOLERANCE_MIN) return 0;
    return age;
  }
  ...
  function slot(key, feed, s, nowMs) {
    ...
    if (key === "v3" && identityReason(feed)) {
      out.why = identityReason(feed);
      return out;
    }
    ...
    // The retained V3 producer declares its trading role as shadow. Its
    // board presentation is primary, independently of that source flag.
    var sourceShadow = def.sourceShadow === undefined ? def.shadow : def.sourceShadow;
    if (sourceShadow && feed.shadow !== true) {
    ...
    var refused = identityReason(feed, r);
    if (key === "v3" && refused) {
      out.why = refused;
      return out;
    }
