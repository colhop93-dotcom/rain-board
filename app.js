...
(function () {
  ...
  var state = null, settle = null, physics = null, models = null, cams = null, metars = null, nextm = null, scorecardData = null, v2rec = null, book = null, candidates = null, physics3 = null, camRings = null, camHealth = null, healthError = null;
  ...
  function phys3For(s) {
    if (phys3Status(s) || phys3Stale()) return null;
    return physics3.stations[s.icao];
  }
  function phys3Pct(s) { var p = phys3For(s); return p ? p.pct : null; }
  function phys3Status(s) {
    if (!physics3) return "v3 unavailable: feed missing";
    if (!physics3.stations || !physics3.stations[s.icao]) return "v3 unavailable: no row for this station";
    var p = physics3.stations[s.icao];
  ...
  function phys3Stale() {
    if (!physics3) return null;
    var age = ageMin(physics3.generated_utc);
    if (age === null) return "v3 unavailable: generation time unknown";
    if (age < -2) return "v3 unavailable: generation time in the future";
    if (age > 90) return "v3 stale " + agoTxt(physics3.generated_utc);
    return null;
  }
  ...
  function ringFreshTxt(c) {
    ...
    parts.push("image capture time: " + (capture ? String(capture) + " (source stamp at last check)" : "unknown"));
    parts.push("last successful check: " + agoTxt((h && h.last_ok_utc) || c.last_ok_utc || (c.verified ? c.checked_utc : null)));
    ...
    if (h && (healthError || (checkAge !== null && checkAge > HEALTH_STALE_MIN))) {
    ...
    if (h && typeof h.stale_min === "number" && (h.changes || 0) >= 1) parts.push(h.stale_min >= 20 ? "picture unchanged " + h.stale_min + " min (may be stale)" : "picture changed " + agoTxt(h.last_changed_utc));
    else if (h && (h.samples || 0) >= 2) parts.push("no change observed across " + h.samples + " checks (" + h.stale_min + " min; may be stale)");
  ...
  function load() {
    return Promise.all([
      ...
      fetchJSON("cam_health.json").catch(function () { return null; })      // WO-134: the shadow challenger, display only
    ]).then(function (r) { state = r[0]; settle = r[1]; physics = r[2]; models = r[3]; if (r[4]) camRings = r[4]; ringsError = r[4] ? null : Date.now(); if (r[12]) camHealth = r[12]; healthError = r[12] ? null : Date.now(); metars = r[5]; nextm = r[6]; v2rec = r[7]; scorecardData = r[8]; book = r[9]; candidates = r[10]; physics3 = r[11]; render(); })
  ...
  function rowHTML(s, w, extra) {
    ...
    return '<div class="row' + (w.overdue ? ' ovl' : '') + (s.icao === liveSel ? ' sel' : '') + '" data-icao="' + s.icao + '">'
      ...
      + '<div class="line timing">' + (p !== null ? 'v2 <b>' + pct(p) + '</b> &middot; ' : '') + esc(timingPhrase(s, w)) + (win && w.k !== "LOCKED" ? ' &middot; rain hours ahead <b>' + esc(win.text) + '</b>' : '') + (phys3Pct(s) !== null ? ' &middot; v3 <b>' + pct(phys3Pct(s)) + '</b>' : (phys3Stale() ? ' &middot; ' + phys3Stale() : (phys3Status(s) ? ' &middot; ' + phys3Status(s) : ''))) + '</div>'
      + (extra || '') + '</div>';
  }
  ...
  function drawOverlays(m, s, store) {
    ...
    var groups = {};
    var ringList = ringsOpen() ? ringsFor(s).filter(ringAvailable) : [];
    ringList.forEach(function (c) {
      if (c.lat === undefined || c.lat === null) return;
      var el = document.createElement("div");
      el.className = "camk ring" + (c.band === "APPROACH" ? " approach" : "");
  ...
  function renderRings(s) {
    ...
    order.forEach(function (sec) {
      var here = list.filter(function (c) { return c.sector === sec; }).sort(function (a, b) { return a.dist_km - b.dist_km; });
      ...
      html += '<div class="sector"><h4><b>' + sec + '</b>' + (here.length ? here.length + ' camera' + (here.length === 1 ? '' : 's') + ', ' + mi(here[0].dist_km) + ' to ' + mi(here[here.length - 1].dist_km) + ' mi out' : 'no verified camera') + '</h4>';
  ...
  function refreshAirport() {
    ...
    renderHours(s);
    renderStack(s, true);
    var near = camsFor(s).slice().sort(function (a, b) { return a.dist_km - b.dist_km; });
  ...
  function renderStack(s, keepOpen) {
    ...
    var md = '<div class="kv">'
      ...
      + '</span>'
      + (function () { var q = phys3For(s); return '<span>v3 shadow</span><span>' + (q ? '<b>' + pct(phys3Pct(s)) + '</b> rest of day (whole day ' + (typeof q.p_day === "number" && isFinite(q.p_day) && q.p_day >= 0 && q.p_day <= 100 ? pct(q.p_day) : 'unavailable') + '), <b>CHALLENGER IN SHADOW</b>: fit ' + esc(q.model_id || '') + ', trained with a precipitation forecast, no live record yet, not traded from' : esc(phys3Stale() || phys3Status(s))) + '</span>'; })()
    ...
    $("ap-stack").innerHTML = sec("Contract", esc(m.ticker || '') + (left !== null && left > 0 ? ", " + Math.floor(left / 60) + " h " + (left % 60) + " m left" : ""), ctr, true)
      ...
      + sec("Models", pct(pp) + " v2" + (phys3Pct(s) !== null ? " &middot; " + pct(phys3Pct(s)) + " v3 shadow" : (phys3Stale() ? " &middot; " + phys3Stale() : (phys3Status(s) ? " &middot; " + phys3Status(s) : ""))), md, true) + sec("Market", "YES " + cents(m.yes_ask), mk, true) + sec("Source health", agoTxt(state.generated_utc), health, false) + sec("Raw", "", raw, false);
  ...
  function camStrip(s, list) {
    ...
    return list.map(function (c, i) {
      ...
      var media = dead ? '<div class="ph">unavailable at the latest check</div>'
        ...
        : camMediaHTML(c, true, true);
      return '<div class="cam' + (dead ? ' dead' : '') + '" data-i="' + i + '">' + media + '<div class="cap"><b>' + esc(c.name) + '</b>' + mi(c.dist_km) + ' mi ' + esc(c.dir || '') + ' of the gauge &middot; ' + (c.type === "youtube" ? "live stream" : "still, refreshes each minute") + '<small>' + esc(ringFreshTxt(c)) + (publicCamURL(c.page || c.url || (c.proxy && c.proxy.src)) ? ' &middot; <a target="_blank" rel="noopener" href="' + esc(publicCamURL(c.page || c.url || c.proxy.src)) + '">source</a>' : '') + '</small>' + '</div></div>';
    }).join("");
  }
