...
(function () {
  ...
  function openCams(city) {
    ...
    list.forEach(function (c) {
      html += '<div class="cam"><div class="camlbl">' + esc(c.name) + (c.dist_km ? ' &middot; ' + esc(c.dist_km) + ' km ' + esc(c.dir || '') : '') + ' from the gauge</div>' + camMediaHTML(c);
      html += '</div>';
    });
    html += '</div>';
    box.innerHTML = html;
  ...
  function buildCamNode(city) {
    ...
    el.innerHTML = '<div class="camlbl"><b>LIVE CAM</b> nearest verified: ' + (c.dist_km ? esc(c.dist_km) + ' km ' + esc(c.dir || '') + ' of the gauge' : '') + (list.length > 1 ? ' <button class="chipbtn camnext">next cam (' + (i + 1) + '/' + list.length + ')</button>' : '') + '<div class="camname">' + esc(c.name) + '</div></div>' + camMediaHTML(c);
