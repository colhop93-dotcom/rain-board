...
(function () {
  ...
  var state = null, settle = null, physics = null, models = null, cams = null, metars = null, nextm = null, scorecardData = null, camHealth = null;
  ...
  function cameraHeld(c) { var h = camHealth && camHealth.cams && camHealth.cams[c.type === "proxy" ? "p:" + c.frame : "i:" + c.url]; return !!c.held || c.verified === false || !!(h && (h.error || h.proxy_error)); }
  function cameraTimes(c) {
    var h = camHealth && camHealth.cams && camHealth.cams[c.type === "proxy" ? "p:" + c.frame : "i:" + c.url];
    return "image capture time: " + ((h && h.source_stamp) || "unknown") + "; last successful check: " + agoTxt((h && h.last_ok_utc) || c.last_ok_utc || c.checked_utc) + "; catalog updated: " + agoTxt(cams && cams.updated_utc);
  }
  function camsFor(s) { return ((cams && cams.near && cams.near[s.city]) || []).map(function (c) { return Object.assign({}, c, {held:cameraHeld(c)}); }); }
  ...
  function load() {
    return Promise.all([
      ...
      fetchJSON("scorecard.json").catch(function () { return null; }), fetchJSON("cam_health.json").catch(function () { return null; })
    ]).then(function (r) { state = r[0]; settle = r[1]; physics = r[2]; models = r[3]; cams = r[4]; metars = r[5]; nextm = r[6]; scorecardData = r[7]; if (r[8]) camHealth = r[8]; render(); })
  ...
  function render() {
    ...
    if (sheet && sheet.__cam) {
      if (!(!cameraHeld(sheet.__cam)) || sheet.__catalog !== (cams && cams.updated_utc)) closeSheet();
      else { var caption = sheet.querySelector("small"); if (caption) caption.textContent = mi(sheet.__cam.dist_km) + " mi " + (sheet.__cam.dir || "") + " of the gauge; " + cameraTimes(sheet.__cam); }
    }
    if (camPopup && camPopup.__cam && cameraHeld(camPopup.__cam)) { camPopup.remove(); camPopup = null; }
  ...
  function refreshAirport() {
    ...
    renderHours(s);
    renderStack(s, true);
    var near = camsFor(s); $("ap-cams").innerHTML = camStrip(s, near); bindCams($("ap-cams"), near);
    panel.scrollTop = y;
  ...
  /* ---------------- cameras ---------------- */
  function camURL(c) {
    if (c.type === "youtube") return "https://www.youtube.com/embed/" + encodeURIComponent(c.id || "") + "?autoplay=1&mute=1";
  ...
  function camStrip(s, list) {
    ...
    return list.map(function (c, i) {
      ...
      var media = dead ? '<div class="ph">unavailable at the latest check</div>'
        ...
        : '<img referrerpolicy="no-referrer" src="' + esc(u) + (u.indexOf("?") > 0 ? "&" : "?") + 't=' + Math.floor(Date.now() / 60000) + '" alt="" loading="lazy">';
      return '<div class="cam' + (dead ? ' dead' : '') + '" data-i="' + i + '">' + media + '<div class="cap"><b>' + esc(c.name) + '</b>' + mi(c.dist_km) + ' mi ' + esc(c.dir || '') + ' of the gauge &middot; ' + (c.type === "youtube" ? "live stream" : "still, refreshes each minute") + '<small>' + esc(cameraTimes(c)) + '</small>' + '</div></div>';
    }).join("");
  }
  function bindCams(root, list) {
    root.querySelectorAll(".cam").forEach(function (el) {
      ...
      ph.onclick = function () {
        if (c.held) return;
        var u = camURL(c);
        ph.outerHTML = c.type === "youtube" ? '<iframe src="' + esc(u) + '" allow="autoplay" loading="lazy"></iframe>' : '<img referrerpolicy="no-referrer" src="' + esc(u) + (u.indexOf("?") > 0 ? "&" : "?") + 't=' + Math.floor(Date.now() / 60000) + '" alt="">';
      };
    });
  }
  var camPopup = null;
  ...
  function openCamPopup(m, s, c, lngLat, exact) {
    ...
    if (camPopup) { camPopup.remove(); camPopup = null; }
    var u = camURL(c), media = c.type === "youtube" ? '<iframe src="' + esc(u) + '" allow="autoplay"></iframe>' : '<img referrerpolicy="no-referrer" src="' + esc(u) + (u.indexOf("?") > 0 ? "&" : "?") + 't=' + Math.floor(Date.now() / 60000) + '" alt="">';
  ...
  function openCamSheet(s, c) {
    if (cameraHeld(c)) return;
    var sh = $("sheet"); sh.classList.remove("hidden"); sh.__cam = c; sh.__catalog = (cams && cams.updated_utc);
    var u = camURL(c);
    sh.innerHTML = '<button class="x">close</button><b>' + esc(c.name) + '</b><br><small>' + mi(c.dist_km) + ' mi ' + esc(c.dir || '') + ' of the gauge; ' + esc(cameraTimes(c)) + '</small><div class="cam" style="margin-top:8px">' + (c.type === "youtube" ? '<iframe src="' + esc(u) + '" allow="autoplay"></iframe>' : '<img referrerpolicy="no-referrer" src="' + esc(u) + '" alt="">') + '</div>';
