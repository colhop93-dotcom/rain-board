(function () {
  ...
  var V3_ROLE = "SHADOW; the model the phone alerts use";
  ...
  function render() {
    ...
    var h = '<div class="model-freshness v3-identity"><b style="font-size:calc(16px * var(--type-scale))">V3: ' + esc(V3_ROLE.toUpperCase()) + '</b>'
    ...
    if (physics3 && physics3.model_id) {
      ...
      h += '<div class="kv" style="margin-bottom: 12px; font-size: var(--fs-small);">'
    ...
    renderSentinel();
    document.getElementById("fresh").textContent = "rendered " + new Date().toLocaleTimeString("en-US", {
      timeZone: "America/Chicago", hour: "numeric", minute: "2-digit"
    }) + " CT" + (physics3 ? "" : ", v3 feed missing");
  }
  ...
  function textSize(value) {
    ...
    document.getElementById('text-size').value=value;
  }
  var savedTextSize;
  try {savedTextSize=localStorage.getItem('rb.type.v1');} catch(e) {savedTextSize=null;}
  textSize(savedTextSize);
  document.getElementById('text-size').onchange=function(){
    textSize(this.value);
    try {localStorage.setItem('rb.type.v1',this.value);} catch(e) { /* Current choice still applies. */ }
  };
  function theme() {
